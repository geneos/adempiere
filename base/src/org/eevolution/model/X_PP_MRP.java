...
public class X_PP_MRP extends PO implements I_PP_MRP, I_Persistent 
{
	...
	public org.eevolution.model.I_PP_MRP getRef_MRP() throws RuntimeException
    {
		return (org.eevolution.model.I_PP_MRP)MTable.get(getCtx(), org.eevolution.model.I_PP_MRP.Table_Name)
			.getPO(getRef_MRP_ID(), get_TrxName());	}

	/** Set Referenced MRP.
		@param Ref_MRP_ID 
		MRP ID
	  */
	public void setRef_MRP_ID (int Ref_MRP_ID)
	{
		if (Ref_MRP_ID < 1) 
			set_Value (COLUMNNAME_Ref_MRP_ID, null);
		else 
			set_Value (COLUMNNAME_Ref_MRP_ID, Integer.valueOf(Ref_MRP_ID));
	}

	/** Get Referenced MRP.
		@return MRP ID
	  */
	public int getRef_MRP_ID () 
	{
		Integer ii = (Integer)get_Value(COLUMNNAME_Ref_MRP_ID);
		if (ii == null)
			 return 0;
		return ii.intValue();
	}
